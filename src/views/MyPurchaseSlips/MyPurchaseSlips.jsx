...
import styles from './MyPurchaseSlips.module.scss';
import PurchaseSlipService from "services/PurchaseSlipService";


@observer
class MyPurchaseSlips extends Component {
  ...
  async getDataFromApi() {
    const formatedDateFrom = this.datesInterval.from === null ? null : this.datesInterval.from.format('YYYY-MM-DD');
    const formatedDateTo = this.datesInterval.to === null ? null : this.datesInterval.to.format('YYYY-MM-DD');
    this.isLoading = true;
  ...
  proceedWithData() {
    this.actualData = (this.dataFromApi && this.dataFromApi.items) ? this.dataFromApi.items : null;
    this.hidePagination = this.actualData.length <= 0;
    this.totalItemsCount = (this.dataFromApi && this.dataFromApi.totalItemsCount) ? this.dataFromApi.totalItemsCount : 0;
  }
  ...
  render() {
    const breadcrumbElements = [{name: 'home', href: '/home'}, {name: 'myPurchaseSlips', isActive: true}];
      ...
      <MainLayout {...this.props}>
        <div className={styles.MyPurchaseSlips}>
          <Container fluid>
            ...
            <Header>{translate('globals.menu.myPurchaseSlips')}</Header>
            <div className={styles.TopBar}>
              ...
              <SearchBoxSmall
                ...
                placeholder={translate('myPurchaseSlips.searchPlaceholder')}
              />
            </div>
...
export default MyPurchaseSlips;
